...
export async function generateMetadata({ params }: { params: Promise<Params> }) {
  try {
    ...
    const { meta } = await loadEntry("lower", slug);
    const img = resolveImageUrl("lower", meta.slug, meta.image);
    return {
      title: `${meta.title} — Lower Extremities`,
...
export default async function LowerEntryPage({ params }: { params: Promise<Params> }) {
  try {
    const { slug } = await params;
    const { meta, html } = await loadEntry("lower", slug);
    const hero = resolveImageUrl("lower", meta.slug, meta.image);
