      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <Image src="/assets/logo.png" width={40} height={40} alt="logo" className="rounded-full shadow-sm" />
            {!collapsed && <span className="font-bold text-blue-700 text-lg">The XPosiGuide</span>}
          </div>

          <button onClick={() => setCollapsed(!collapsed)} className="text-slate-500 hover:text-blue-600 text-xl">
            {collapsed ? "›" : "‹"}
          </button>
        </div>
          ...
          {navLinks.map((item) => (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium
                ${
                  pathname === item.href
                    ? "bg-blue-100/70 border-l-4 border-blue-600 text-blue-700 shadow-inner"
                    : "border-l-4 border-transparent text-slate-600 hover:bg-white/70 hover:shadow-sm"
                }`}
            >
              {item.icon}
