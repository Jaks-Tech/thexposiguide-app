    ...
    <header className="sticky top-0 z-50 bg-gradient-to-r from-blue-800 via-blue-700 to-gray-800 text-white shadow-lg">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8 py-3">
        <Link href="/" className="flex items-center gap-3">
          <Image
            ...
            priority
          />
          <span className="text-lg sm:text-xl font-bold tracking-wide">The XPosiGuide</span>
        </Link>

        {/* Desktop links */}
        <ul className="hidden md:flex flex-wrap gap-3" /* key forces remount when route changes */ key={pathname}>
          {navLinks.map((link) => {
        ...
        >
          <span className={`block h-0.5 w-5 bg-white transition ${open ? "rotate-45 translate-y-1" : ""}`} />
          <span className={`block h-0.5 w-5 bg-white my-1 transition ${open ? "opacity-0" : ""}`} />
          <span className={`block h-0.5 w-5 bg-white transition ${open ? "-rotate-45 -translate-y-1" : ""}`} />
        </button>
      </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-gradient-to-b from-blue-800/95 to-gray-900/95 backdrop-blur-sm">
          <ul className="mx-auto flex max-w-6xl flex-col gap-2 px-4 sm:px-6 lg:px-8 py-3" key={pathname}>
            {navLinks.map((link) => {
              const isActive = mounted && pathname === link.href;
              return (
                <li key={link.href}>
